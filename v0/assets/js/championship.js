...
var winners;
d3.csv(
  './assets/data/winners_annual.csv',
  converter2,
  data => (winners = data)
);

function draw() {
  d3.csv('./assets/data/teams_active_years.csv', converter, dataset => {
    if (window.innerWidth > 800) {
      ...
      var x_axis = svg
        ...
        .attr(
          'transform',
          `rotate(-45) translate(${(1 * margin.left) / 2}, ${-10})`
        )
        .attr('class', 'axis_text')
    ...
    champions
      .on('mouseover', d => {
        ...
        div
          .html(
            '<strong>' + d.year + '</strong><br><span>' + d.driver + '</span>'
          )
          // .style("width", d.driver.length*100 + "px")
          .style('left', () => {
            let x = d3.event.pageX;
            let bar = window.innerWidth * 0.5;
            if (x < bar) {
              return x + 'px';
            } else {
              return x - 550 / 3 + 'px';
