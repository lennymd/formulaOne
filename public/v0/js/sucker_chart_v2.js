class sucker_chart {
  ...
  organizeData () {
    const sorted_data = this.base_data.sort(
      (b, a) => b[this.rank] - a[this.rank]
    );
  ...
  create_scales () {
    ...
    if (this.normalize) {
      ...
      k = Math.floor(d3.max(this.data, d => d[this.x]) * 1.2);
    }
  ...
  create_axes () {
    ...
    this.x_axis = this.svg
      .append('g')
      .attr('class', 'x axis')
      .call(d3.axisTop(this.x_scale).tickFormat(d3.format(tick_format)))
      .selectAll('text')
  ...
  update (column, normalize, filter) {
    ...
    var lines = this.svg.selectAll('.line').data(this.data);
